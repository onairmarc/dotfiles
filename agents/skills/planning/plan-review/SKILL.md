...
Read and follow `~/.agents/skills/delivery-constraints/SKILL.md`. A plan is not agent-ready unless it is structured as vertical slices, assumes the work lands in
...
Follow the standards-discovery procedure in `~/.agents/skills/planning-commons/paths.md` and record the extracted rules as `$PROJECT_STANDARDS` — the checklist
