...
`react.md`) runs its own `5a` detection, sets the handful of parameters below, then follows this procedure. Wherever a `{{PARAM}}` appears, substitute the value the stack
file provides:
