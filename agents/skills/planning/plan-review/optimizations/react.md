...
Detection is done by the caller when the TanStack check did **not** match, so if you reached this file, React is present and TanStack is not. Continue.
...
- `{{FIX_EXAMPLES}}` = extract an inline component to module scope, replace `.sort(` on props with `toSorted()`, lazy-load a heavy editor with `React.lazy`
