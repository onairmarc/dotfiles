...
Read and follow `~/.agents/skills/delivery-constraints/SKILL.md`. Every plan this skill produces must be structured as vertical slices, must assume the work
...
`~/.agents/skills/delivery-constraints/SKILL.md`; they are enforced by Lens E in Step 2 and reproduced verbatim in the plan itself (see the plan's `## Delivery
constraints` section).
...
3. **Discover project standards & policies (mandatory)** — the plan must not violate a single documented policy, so finding them is not optional. Follow the
   standards-discovery procedure in `~/.agents/skills/planning-commons/paths.md` and record the extracted rules as `$PROJECT_STANDARDS`, the checklist the plan
...
4. **Find northstar** — resolve `$NORTHSTAR` per the `$NORTHSTAR` ladder in `~/.agents/skills/planning-commons/paths.md`. If none is found, `$NORTHSTAR = null`
...
run the interactive review loop in `~/.agents/skills/planning-commons/review-loop.md`: batch at most 4 questions per call ranked by blast radius, write every
