...
description: Canonical shared references for the planning skills — path resolution and standards discovery, the question review loop and BLOCK/WARN severity, the plan/sub-plan document format, the parallel-execution overlay, and todowrite task tracking. Read the relevant file at the start of any planning skill that resolves repo paths, runs an interactive review loop, emits or consumes a plan file, runs the parallel branch of plan-split or plan-execute, or tracks skill steps to completion, so the rule lives in one place instead of being restated per skill.
---
...
it, exactly as skills already load `delivery-constraints` and `file-operations`.
...
| Interactive review loop + BLOCK/WARN severity     | [`review-loop.md`](review-loop.md)               | a skill asks the user via `question`, iterates lenses until clean, or classifies a finding as BLOCK/WARN.                  |
...
| Plan-file deletion authority                      | [`plan-file-deletion.md`](plan-file-deletion.md) | `plan-split` emits sub-plans, or `plan-execute` creates `.agent-instructions.md` or deletes a plan directory.              |
