...
Tenant context is resolved once and persisted automatically via Laravel's Context system. It propagates throughout the entire request or job lifecycle without needing to
be re-applied.
...
- **Command loops over multi-tenant data**: this is the only case where you need to call
  `TenantContext::applyById($tenantId, true)` — once per record to switch the context as you iterate across different tenants.
