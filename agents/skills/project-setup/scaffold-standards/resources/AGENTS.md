...
> User-level skills live in `~/.agents/skills/` (i.e., `C:\Users\<username>\.config\opencode\skills\` on Windows
> or `~/.agents/skills/` on Linux/macOS) and are **not** listed here. Before concluding a skill does not exist, check the
