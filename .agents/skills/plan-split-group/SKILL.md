...
- **The `## Delivery constraints` section is mandatory** in every sub-epic `plan.md`, placed before Architecture, and must be filled in with the project's real runner
  command rather than left as a template. Never replace it with a pointer to the master plan or to `.agents/skills/delivery-constraints/SKILL.md` — each sub-epic must
